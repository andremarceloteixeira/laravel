...
class Carbon extends DateTime
{
   ...
   */
   ...
   public function diffForHumans(Carbon $other = null)
   {
      ...
      $unit = trans_choice('carbon.year',$delta);

      $count = 0;
      foreach ($divs as $divValue) {
         if ($delta < $divValue) {
            $unit = $count;
            break;
         }
      ...
      $divs = array(
         trans_choice('carbon.second',$delta),
         trans_choice('carbon.minute',$delta),
         trans_choice('carbon.hour',$delta),
         trans_choice('carbon.day',$delta),
         trans_choice('carbon.week',$delta),
         trans_choice('carbon.month',$delta)
      );
      ...
      if ($isNow) {
         if ($isFuture) {
            return trans_choice('carbon.from_now', $delta, array('hour' => $txt));
         }

         return trans('carbon.ago', array('hour' => $txt));
      }

      if ($isFuture) {
         return trans('carbon.before', array('hour' => $txt));
      }

      return trans('carbon.after', array('hour' => $txt));
   }
